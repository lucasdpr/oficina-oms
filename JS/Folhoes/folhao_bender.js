...
// 🔧 CORREÇÃO: removida window.abrirFolhaoBender(id), uma segunda
// implementação de abertura do Bender que nunca é chamada em lugar
// nenhum do sistema (nenhum HTML/onclick a referencia) — quem abre o
// Bender de verdade é abrirFolhaoMCC4() em folhaoMolde4.js, que trata o
// caso "BENDER" internamente e SIM chama a persistência de rascunho
// (restaurarRascunhoNoModal/ativarAutoSalvamentoFolhao/finalizarRascunhoFolhao).
// Essa função órfã não chamava nenhuma delas — era uma armadilha: se
// algum onclick fosse trocado no futuro pra chamar window.abrirFolhaoBender
// direto (o nome sugere que era essa a intenção original), o rascunho do
// Bender deixaria de ser salvo/restaurado, silenciosamente. Removida em
// vez de mantida como código morto — o fluxo real (abrirFolhaoMCC4) já
// cobre o Bender.