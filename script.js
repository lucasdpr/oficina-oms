// ==========================================================================
// BANCO DE DADOS CORE - SISTEMA OMS (VERSÃO COMPLETA DE PRODUÇÃO v28 MOBILE)
// ==========================================================================
let BANCO_ATIVOS = JSON.parse(localStorage.getItem("oms_ativos_v28_local"));
let HISTORICO_ACOES = JSON.parse(localStorage.getItem("oms_historico_v28_local")) || [];
...
if (!BANCO_ATIVOS || BANCO_ATIVOS.length < 150) {
  ...
  veiosMcc4.forEach(veio => {
    ...
    for (let b = 1; b <= 5; b++) BANCO_ATIVOS.push({ id: `BOW-${b}-4${veio}`, tipo: "Bow", local: vNome, pos: `Curvo Bow #0${b}`, dias: 60, ton: 650000, meta: 1600000, ordem: 300 + b });
    for (let s = 1; s <= 2; s++) BANCO_ATIVOS.push({ id: `STR-${s}-4${veio}`, tipo: "Straightener", local: vNome, pos: `Endireitador #0${s}`, dias: 88, ton: 910000, meta: 1800000, ordem: 400 + s });
    for (let h = 1; h <= 10; h++) BANCO_ATIVOS.push({ id: `HOR-${h}-4${veio}`, tipo: "Horizontal", local: vNome, pos: `Horizontal #0${h}`, dias: 102, ton: 430000, meta: 2000000, ordem: 500 + h });
  });
  ...
  localStorage.setItem("oms_ativos_v28_local", JSON.stringify(BANCO_ATIVOS));
}

let EM_EMERGENCIA = JSON.parse(localStorage.getItem("oms_emergencia_v28_local")) || null;
let OPERADOR_LOGADO = JSON.parse(localStorage.getItem("oms_operador_v28_local")) || null;
let VEIO_SELECIONADO_PAINEL = "C";

const CADASTRO_MATRICULAS = {
  "40090430": "Filipe (Líder)", "40075827": "Denilson (Líder)",
  "40080751": "Valmir (Líder)", "40090851": "Samuel (Líder)", "1011": "Supervisor"
};
...
function processarAutenticacaoHome() {
  ...
  if (CADASTRO_MATRICULAS[matriculaInput]) {
    ...
    localStorage.setItem("oms_operador_v28_local", JSON.stringify(OPERADOR_LOGADO));
    ...
    atualizarInterfaceUsuario(); registrarHistorico("AUTENTICAÇÃO", `Login executado com sucesso.`);
    calcularKpisGlobais(); renderPainelVeios(); renderAtivos(); renderReparos(); renderReservas();
  } else {
...
function registrarHistorico(tag, acao) {
  ...
  localStorage.setItem("oms_historico_v28_local", JSON.stringify(HISTORICO_ACOES));
  renderHistorico();
}
...
function renderHistorico() {
  ...
  if (!tbody) return;
  tbody.innerHTML = HISTORICO_ACOES.map(h => `<tr><td><small class="text-muted">${h.data}</small></td><td><span class="ind-card-tag bg-tag">${h.tag}</span></td><td>${h.acao}</td><td><small>${h.responsavel}</small></td></tr>`).join("");
}

function fazerLogout() {
  if (confirm("Encerrar o turno?")) {
    registrarHistorico("SISTEMA", "Turno encerrado.");
    OPERADOR_LOGADO = null; localStorage.removeItem("oms_operador_v28_local");
...
function mudarVeioVisualizado(veioNome) {
  ...
  renderPainelVeios(); renderAtivos();
}
...
function calcularKpisGlobais() {
  ...
  BANCO_ATIVOS.forEach(a => {
    ...
    if (pct >= 80 && !a.local.includes("Oficina")) c++;
...
function renderPainelVeios() {
  ...
  if (ativos.length === 0) return container.innerHTML = `<div class="vazio">Nenhum componente instalado no Veio ${VEIO_SELECIONADO_PAINEL}.</div>`;
...
function gerarCardGraficoHTML(a) {
  ...
  return `
    <div class="mcc-grafico-card premium-shadow" style="border-top: 3px solid ${cor};">
      ...
      <div class="ind-gauge-bar premium-bar"><div class="ind-gauge-fill" style="width:${Math.min(pct,100)}%; background:${cor};"></div></div>
      <div class="grafico-legenda" style="margin-bottom: 10px;"><span>Ton: <strong>${Math.round(a.ton).toLocaleString()}</strong></span><span>Lim: ${a.meta.toLocaleString()}</span></div>
      <button class="btn-xs-primary w-100" style="border: 1px dashed var(--text-accent); color: var(--text-accent); background: rgba(56,189,248,0.05); padding: 8px; border-radius: 4px; cursor: pointer;" onclick="abrirHistoricoIndividual('${a.id}')"><i class="fas fa-book-open"></i> Ver Prontuário</button>
    </div>`;
}
...
function fecharModalHistorico() {
  ...
  ID_HISTORICO_ATUAL = null; document.getElementById("input-nota-manual").value = "";
}
...
function renderizarTabelaHistoricoIndividual(id) {
  ...
  tbody.innerHTML = historicoFiltrado.map(h => `<tr><td style="font-size: 11px; white-space: nowrap; color: var(--text-muted);">${h.data}</td><td style="font-size: 13px;">${h.acao}</td><td style="font-size: 11px; color: var(--text-accent);">${h.responsavel}</td></tr>`).join("");
}
...
function salvarRegistroManual() {
  ...
  document.getElementById("input-nota-manual").value = ""; renderizarTabelaHistoricoIndividual(ID_HISTORICO_ATUAL);
}
...
function fecharModalRelatorio() { document.getElementById('modal-relatorio').classList.add('hidden'); MODO_MODAL_RELATORIO = {}; }
...
function confirmarRelatorio() {
  ...
  if (MODO_MODAL_RELATORIO.tipoAcao === 'SAQUE') executarSaqueFinal(MODO_MODAL_RELATORIO.idSacado, textoLaudo);
  else if (MODO_MODAL_RELATORIO.tipoAcao === 'SWAP') executarSwapFinal(MODO_MODAL_RELATORIO.idReserva, MODO_MODAL_RELATORIO.idSacado, MODO_MODAL_RELATORIO.localDestino, textoLaudo);
...
function executarSaqueFinal(id, laudo) {
  let item = BANCO_ATIVOS.find(a => a.id === id);
  if (item) { 
    let loc = item.local; item.local = "Oficina / Reparo"; 
    localStorage.setItem("oms_ativos_v28_local", JSON.stringify(BANCO_ATIVOS)); 
...
function fecharModalConcluirReparo() { document.getElementById("modal-concluir-reparo").classList.add("hidden"); ID_REPARO_ATUAL = null; }
...
function confirmarConclusaoReparo() {
  ...
  if(tipo === "GERAL") {
    item.ton = 0; item.dias = 0; msgHistorico = "Reparo GERAL finalizado. Tonelagem zerada.";
  } else {
    ...
    item.ton = novaTon; item.dias = novosDias; msgHistorico = `Reparo PARCIAL finalizado. Retorna com ${novaTon}t e ${novosDias} dias.`;
  }

  item.local = "Oficina / Reserva"; 
  localStorage.setItem("oms_ativos_v28_local", JSON.stringify(BANCO_ATIVOS));
  registrarHistorico(item.id, msgHistorico);
  fecharModalConcluirReparo(); renderReparos(); renderReservas(); renderAtivos(); calcularKpisGlobais();
}
        ...
        <tr>
          ...
          <td>
            <div class="flex-align-center gap-10">
              ...
              <div class="ind-gauge-bar premium-bar w-100px"><div class="ind-gauge-fill bg-danger" style="width: ${Math.min(pct, 100)}%;"></div></div>
            </div>
          </td>
          <td>
  ...
  if (pecaReserva) {
    ...
    if (pecaAntiga) {
      ...
      if (confirm(`Instalar a reserva ${pecaReserva.id} no ${novoLocal}?`)) {
        pecaReserva.local = novoLocal; pecaReserva.pos = "Componente Instalado"; pecaReserva.ordem = getOrdemPadrao(pecaReserva.tipo); 
        localStorage.setItem("oms_ativos_v28_local", JSON.stringify(BANCO_ATIVOS));
...
function executarSwapFinal(idReserva, idAntiga, novoLocal, laudo) {
  ...
  if (pecaAntiga && pecaReserva) {
    ...
    pecaReserva.local = novoLocal; pecaReserva.pos = pecaAntiga.pos; pecaReserva.ordem = pecaAntiga.ordem; 
    localStorage.setItem("oms_ativos_v28_local", JSON.stringify(BANCO_ATIVOS));
...
function toggleFormAdicionar() { document.getElementById("form-novo-equipamento").classList.toggle("hidden"); }
...
function salvarNovoEquipamento() {
  ...
  BANCO_ATIVOS.push({ id: tag, tipo: tipo, local: "Oficina / Reserva", pos: "Estoque", dias: 0, ton: 0, meta: meta, ordem: getOrdemPadrao(tipo) });
  localStorage.setItem("oms_ativos_v28_local", JSON.stringify(BANCO_ATIVOS));
...
function fazerCelulaEditavel(celula, id, campo) {
  ...
  celula.innerHTML = ""; celula.appendChild(input); input.focus();

  input.addEventListener("blur", () => {
    let val = campo === 'id' ? input.value.trim().toUpperCase() : parseFloat(input.value) || 0;
    let item = BANCO_ATIVOS.find(a => a.id === id);
    if (item && val !== "") { 
      let ant = item[campo]; item[campo] = val; 
      localStorage.setItem("oms_ativos_v28_local", JSON.stringify(BANCO_ATIVOS)); 
...
// NOVA LÓGICA DE FILTROS CRUZADOS (VEIO + STATUS)
...
function renderizarGraficosMCC(mccNumero) {
  ...
  // Lê os botões que estão com a classe 'active' atualmente na tela
  ...
  if(filtrados.length === 0) {
    return container.innerHTML = `<div class="vazio">Nenhum equipamento encontrado com a combinação de filtros selecionada.</div>`;
  }
...
function dispararEmergencia() {
  ...
  localStorage.setItem("oms_emergencia_v28_local", JSON.stringify(EM_EMERGENCIA));
  registrarHistorico("ALERTA", "Botão de Pânico acionado."); exibirBarraEmergencia();
}
function encerrarEmergencia() { EM_EMERGENCIA = null; localStorage.removeItem("oms_emergencia_v28_local"); document.getElementById("barra-emergencia").style.display = "none"; registrarHistorico("ALERTA", "Alarme resetado.");}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (OPERADOR_LOGADO) {
    ...
    atualizarInterfaceUsuario(); calcularKpisGlobais(); renderPainelVeios(); renderAtivos(); renderReparos(); renderReservas();
  }
});