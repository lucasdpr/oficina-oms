// ==========================================================================
// BANCO DE DADOS CORE - SISTEMA OMS (VERSÃO v33 - COMPLETA E EXPANDIDA)
// ==========================================================================
...
// ==========================================================================
// ARRAYS DE CHECKLIST DO FOLHÃO MCC4
// ==========================================================================
...
function abrirAba(event, idAba) {
    ...
    if (event) document.getElementById(event.currentTarget.id).classList.add("active");
    ...
    if (window.innerWidth <= 992) document.getElementById('sidebar-menu').classList.remove('open');
}
...
function registrarHistorico(tag, acao) {
    ...
    HISTORICO_ACOES.unshift({ data: data, tag: tag, acao: acao, responsavel: OPERADOR_LOGADO ? OPERADOR_LOGADO.nome : "Sistema" });
    if (HISTORICO_ACOES.length > 2000) HISTORICO_ACOES.pop();
...
function renderAtivos() {
    ...
    if (filtroEl.value === "Oficina / Reparo") f = BANCO_ATIVOS.filter(a => a.local === "Oficina / Reparo");
    else if (filtroEl.value === "Oficina / Reserva") f = BANCO_ATIVOS.filter(a => a.local === "Oficina / Reserva");
    else if (filtroEl.value !== "TODOS") f = f.filter(a => a.tipo === filtroEl.value);
...
function fazerCelulaEditavel(celula, id, campo) {
    ...
    input.addEventListener("blur", () => {
        ...
        renderAtivos(); renderPainelVeios(); calcularKpisGlobais(); renderReparos(); renderReservas();
    });
}
...
function renderizarGraficosMCC(mccNumero) {
    ...
    if (veioAtivo !== 'TODOS') filtrados = filtrados.filter(a => a.local.includes(`Veio ${veioAtivo}`));
...
function salvarRegistroManual() {
    ...
    if (!nota) return alert("Escreva algo para registrar.");
...
// ==========================================
// SAQUE, REPARO E SWAP (FLUXO PRINCIPAL CORRIGIDO)
// ==========================================
...
function confirmarRelatorio() {
    ...
    if (!condicao) return alert("Por favor, descreva como o equipamento chegou na oficina (Laudo Visual).");
...
function renderReservas() {
    ...
    if (reservas.length === 0) {
        ...
        resBody.innerHTML = reservas.map(a => {
            ...
            let optionsVeios = "";
            if (a.mcc_compat === "2/3") {
                optionsVeios = `<option value="MCC 2 - Veio C">Veio C (MCC 2)</option><option value="MCC 2 - Veio D">Veio D (MCC 2)</option><option value="MCC 3 - Veio E">Veio E (MCC 3)</option><option value="MCC 3 - Veio F">Veio F (MCC 3)</option>`;
            } else if (a.mcc_compat === "4") {
                optionsVeios = `<option value="MCC 4 - Veio H">Veio H (MCC 4)</option><option value="MCC 4 - Veio G">Veio G (MCC 4)</option>`;
            }
            return `
                <tr>
                    <td class="font-code">${a.id}</td>
                    ...
                    <td>
                        <div class="flex-align-center gap-10 action-buttons-mobile">
                            <select id="alocar-veio-${a.id}" class="premium-select select-sm">${optionsVeios}</select>
    ...
    if (pecaReserva) {
        ...
        if (pecaAntiga) {
            ...
            if (confirm(`Instalar a reserva ${pecaReserva.id} no ${novoLocal}?`)) {
                ...
                renderReparos(); renderReservas(); renderAtivos(); renderPainelVeios(); calcularKpisGlobais();
            }
        }
    }
}

// CORRIGIDO O NOME DA FUNÇÃO DE SWAP (Antes estava ejecutarSwapFinal, o que causava a falha)
...
function abrirModalConcluirReparo(id) {
    ...
    if (item.tipo === "Molde" && item.mcc_compat.includes("4")) {
        abrirFolhaoMCC4(id); // Chama o novo sistema super completo
    } else {
        ID_REPARO_ATUAL = id;
...
function toggleCamposReparoParcial() {
    ...
    if (tipo === "PARCIAL") divCampos.classList.remove("hidden");
    else divCampos.classList.add("hidden");
}
...
function confirmarConclusaoReparo() {
    ...
    if (tipo === "GERAL") {
        ...
        item.ton = novaTon; item.dias = novosDias;
    ...
    renderReparos(); renderReservas(); renderAtivos(); calcularKpisGlobais();
}
    ...
    if(!document.getElementById('tab-peritagem-mcc4')) {
        ...
        if(tabsContainer && bodyContainer) {
            tabsContainer.innerHTML += `
                <button id="tab-peritagem-mcc4" class="folhao-tab" onclick="trocarAbaFolhao(event, 'folhao-aba-peritagem')">6. Peritagem (Medidas)</button>
                <button id="tab-eletrica-mcc4" class="folhao-tab" onclick="trocarAbaFolhao(event, 'folhao-aba-eletrica')">7. Elétrica & Termopares</button>
                <button id="tab-materiais-mcc4" class="folhao-tab" onclick="trocarAbaFolhao(event, 'folhao-aba-materiais')">8. Materiais Utilizados</button>
            `;
            
            bodyContainer.innerHTML += `
                <div id="folhao-aba-peritagem" class="folhao-content hidden">
                    <h3 style="margin-bottom: 15px; color: var(--text-heading);">Peritagem e Identificações (Redutores/Cilindros)</h3>
                    <div class="form-grid-4">
                        <div class="input-group"><label>Placa Fixa (Saída Maq)</label><input type="text" id="p-fixa-sm" placeholder="Ex: 1323"></div>
                        <div class="input-group"><label>Placa Fixa (Oficina)</label><input type="text" id="p-fixa-of" placeholder="Ex: 1323"></div>
                        <div class="input-group"><label>Redutor Sup. Dir.</label><input type="text" id="red-sd"></div>
                        <div class="input-group"><label>Cilindro Sup. Dir.</label><input type="text" id="cil-sd"></div>
                    </div>
                    <hr style="border: 1px solid var(--border-color); margin: 20px 0;">
                    <h3 style="margin-bottom: 15px; color: var(--text-heading);">Peritagem Placas Estreitas (Ajustes de Chavetas)</h3>
                    <div class="form-grid-4">
                        <div class="input-group"><label>Folga Aresta - Largura Ref</label><select id="folga-largura" class="premium-select"><option>830</option><option>870</option><option>950</option><option>1030</option><option>1100</option><option>1180</option><option>1230</option><option>1300</option><option>1380</option><option>1460</option><option>1500</option><option>1530</option><option>1550</option><option>1580</option><option>1620</option></select></div>
                        <div class="input-group"><label>Folga Dir (Fixo)</label><input type="text" id="folga-dir-fixo"></div>
                        <div class="input-group"><label>Folga Esq (Móvel)</label><input type="text" id="folga-esq-mov"></div>
                        <div class="input-group"><label>Status Chavetas</label><input type="text" id="chavetas-status" placeholder="Ex: -0.10"></div>
                    </div>
                </div>

                <div id="folhao-aba-eletrica" class="folhao-content hidden">
                    <h3 style="margin-bottom: 15px; color: var(--text-heading);">Elétrica - Sensor de Nível e Resistência de Placas</h3>
                    <div class="form-grid-4">
                        <div class="input-group"><label>Resistência Pinos 1-2 (Ω)</label><input type="text" id="res-1-2" placeholder="Ex: 224,9"></div>
                        <div class="input-group"><label>Resistência Pinos 3-4 (Ω)</label><input type="text" id="res-3-4" placeholder="Ex: 1,2"></div>
                        <div class="input-group"><label>Isolação Pinos 5 e 6 (MΩ)</label><input type="text" id="iso-5-6" placeholder=">10 MΩ"></div>
                        <div class="input-group"><label>Isolação Pinos 8 e 10 (MΩ)</label><input type="text" id="iso-8-10"></div>
                    </div>
                    <hr style="border: 1px solid var(--border-color); margin: 20px 0;">
                    <div class="form-grid-4">
                        <div class="input-group"><label>Média Termopares Placa Fixa (Ω)</label><input type="text" id="termo-fixa" placeholder="10 a 20 Ω"></div>
                        <div class="input-group"><label>Média Termopares Placa Móvel (Ω)</label><input type="text" id="termo-movel" placeholder="10 a 20 Ω"></div>
                        <div class="input-group"><label>Termopares Lado Esquerdo (Ω)</label><input type="text" id="termo-esq" placeholder="5 a 15 Ω"></div>
                        <div class="input-group"><label>Termopares Lado Direito (Ω)</label><input type="text" id="termo-dir" placeholder="5 a 15 Ω"></div>
                    </div>
                </div>
                ...
                <div id="folhao-aba-materiais" class="folhao-content hidden">
                    ...
                    <textarea id="materiais-utilizados-texto" class="premium-textarea" rows="6" placeholder="Liste as quantidades e materiais utilizados. Ex: 4 Parafusos sextavados M24x140; 8 Arruelas Pressão; Massa de calafetar..."></textarea>
                </div>
            `;
        }
    }
...
function abrirFolhaoMCC4(id) {
    injetarAbasFaltantes(); // Garante que a UI gigante do PDF exista
    ID_FOLHAO_ATUAL = id;
...
function salvarEImprimirFolhaoMCC4() {
    ...
    registrarHistorico(item.id, `Folhão Liberatório MCC4 assinado. Execução: ${tipoExecucao}. Motivo: ${motivo}. ${linkImprimir}`);
...
function imprimirLaudoSalvo(tag, motivo) {
    ...
    let pFixaSm = document.getElementById("p-fixa-sm") ? document.getElementById("p-fixa-sm").value : "";
    let resTermo = document.getElementById("termo-fixa") ? document.getElementById("termo-fixa").value : "";
    let materiais = document.getElementById("materiais-utilizados-texto") ? document.getElementById("materiais-utilizados-texto").value : "Nenhum extra informado.";
    
    // Constrói a super tabela de impressão baseada nas especificações do PDF!
    let html = `
        <div class="print-grid">
            <div><strong>TAG do Equipamento:</strong> ${tag}</div>
            <div><strong>Data de Emissão:</strong> ${new Date().toLocaleDateString('pt-BR')} ${new Date().toLocaleTimeString('pt-BR')}</div>
            <div><strong>Motivo Técnico da Manutenção:</strong> ${motivo}</div>
            <div><strong>Líder/Operador Responsável:</strong> ${OPERADOR_LOGADO ? OPERADOR_LOGADO.nome : 'Sistema'}</div>
        </div>
        
        <div class="print-section-title">Dados de Identificação e Rastreabilidade</div>
        <table class="print-table">
            <thead><tr><th>Componente</th><th>Status / Leitura</th><th>Redutores / Cilindros Associados</th></tr></thead>
            <tbody>
                <tr><td>Placa Fixa Saída Maquina</td><td>${pFixaSm || 'OK'}</td><td>Sup. Dir. Conforme</td></tr>
                <tr><td>Média de Leitura Termopares</td><td>${resTermo || '10 a 20 Ω (Padrão)'}</td><td>Calibração Eletrônica OK</td></tr>
            </tbody>
        </table>

        <div class="print-section-title">Resumo Técnico de Conformidade do Laudo (Amostragem Padrão)</div>
        <table class="print-table">
            <thead><tr><th>Item</th><th>Descrição de Verificação Crítica da Manutenção</th><th>Resultado</th></tr></thead>
            <tbody>
                <tr><td>1</td><td>Teste de estanqueidade de água com pressão estabilizada de 10 KGF/cm2 por 30 minutos</td><td>CONFORME [ x ] | REJEITADO [ ]</td></tr>
                <tr><td>2</td><td>Inspeção estrutural das juntas de expansão das placas de cobre principais</td><td>CONFORME [ x ] | REJEITADO [ ]</td></tr>
                <tr><td>3</td><td>Montagem, centralização e calibração fina eletrônica do sensor de nível Vuhz</td><td>CONFORME [ x ] | REJEITADO [ ]</td></tr>
                <tr><td>4</td><td>Centro físico gravado do molde identificado na placa norte e visível ao operador</td><td>CONFORME [ x ] | REJEITADO [ ]</td></tr>
                <tr><td>5</td><td>Aferição milimétrica de folgas nas arestas das placas estreitas e largas (<= 0,35mm)</td><td>CONFORME [ x ] | REJEITADO [ ]</td></tr>
                <tr><td>6</td><td>Calafetação e fixação final de todas as tampas de proteção mecânica</td><td>CONFORME [ x ] | REJEITADO [ ]</td></tr>
            </tbody>
        </table>

        <div class="print-section-title">Relação de Materiais Utilizados</div>
        <div style="border: 1px solid #000; padding: 10px; font-size: 12px; min-height: 50px;">
            ${materiais.replace(/\n/g, "<br>")}
        </div>

        <p style="font-size: 11px; text-align: center; margin-top: 30px;">* Este documento é a cópia impressa do prontuário digital assinado e rastreado no ecossistema OMS de Auditoria Industrial.</p>
    `;
...
function salvarNovoEquipamento() {
    ...
    BANCO_ATIVOS.push({ id: tag, tipo: tipo, local: "Oficina / Reserva", pos: "Estoque", dias: 0, ton: 0, meta: meta, ordem: getOrdemPadrao(tipo), mcc_compat: mcc_compat });
...
// ==========================================
// ESTOQUE DE ROLOS (ALMOXARIFADO ROLOS)
// ==========================================
...
// ==========================================
// ALMOXARIFADO DE MATERIAIS GERAIS (PLANILHA)
// ==========================================
